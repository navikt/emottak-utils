...
class VaultUtil private constructor() {
    ...
    companion object {
        ...
        fun getVaultCredential(
            envVarVaultPath: String,
            ...
            val username =
                vaultData["username"]
                    ?: throw RuntimeException("Failed to read 'username' from Vault (path: '$path'). Available keys: ${vaultData.keys}")
            val password =
                vaultData["password"]
                    ?: throw RuntimeException("Failed to read 'password' from Vault (path: '$path'). Available keys: ${vaultData.keys}")
        ...
        private fun getProperty(propertyName: String): String? = System.getProperty(propertyName, System.getenv(propertyName))
...
fun String.parseVaultJsonObject(field: String) =
    Json
        .parseToJsonElement(
            this,
        ).jsonObject[field]!!
